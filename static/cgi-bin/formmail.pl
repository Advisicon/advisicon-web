...
sub return_html {
    ...
    # Otherwise, begin printing the response page.                           #
        ...
        <center><font size=-1>FormMail V1.92.1</font></center>
        </body>
       </html>
(END HTML FOOTER)
    }
...
sub send_mail {
   # Localize variables used in this subroutine.                            #
    ...
    my($oldfh) = select(MAIL); $| = 1; select($oldfh);

    $_ = <MAIL>; if (/^[45]/) { close MAIL; return; }
    if ($debug){push @ta,$_;}

    print MAIL "helo localhost\r\n";
    $_ = <MAIL>; if (/^[45]/) { close MAIL; return; }
    if ($debug){push @ta,$_;}
    push @ta,$_;    push @ta,"hi";

    print MAIL "mail from: $Config{'email'}\r\n";
    $_ = <MAIL>; if (/^[45]/) { close MAIL; return -5; }
    if ($debug){push @ta,$_;}
    if ($debug){
      print MAIL "rcpt to: andrews\@ltinet.net\r\n";
    } else {
      @rcptpairs = split(/,/, $Config{'recipient'});
      foreach $rcptpair (@rcptpairs) {
	print MAIL "rcpt to: $rcptpair\r\n";
#	print MAIL "rcpt to: $Config{'recipient'}\r\n";
      }
    }
    $_ = <MAIL>; if (/^[45]/) { close MAIL; return -6; }
    if ($debug){push @ta,$_;}
    ...
    if ($debug){push @ta,$_;}
    if ($debug){
      print MAIL "To: andrews\@ltinet.net\r\n";
    } else {
...
<html>
 <head>
  <title>Test Routine</title>
 </head>
 <body bgcolor=#FFFFFF text=#000000>
  <center>
   <table border=0 width=600 bgcolor=#9C9C9C>
    <tr><th><font size=+2>Test Routine</font></th></tr>
   </table>
  ...
  <center>
   ...
   <table border=0 width=600 bgcolor=#CFCFCF>
    <tr><th><tt><font size=+1>Copyright 1995 - 2002 Matt Wright<br>
        Version 1.92.1 - Released April 21, 2002<br>
